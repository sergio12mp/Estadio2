    ...
    try {
        ...
        for (const row of rows) {
            ...
            for (const estadistica of estadisticas) {
                ...
                try {
                    // Obtener idJugador de la base de datos (incluso si ya existía o se insertó)
                    ...
                    if (jugadorDBRows && jugadorDBRows.length > 0) {
                        ...
                        console.error(`No se encontró el ID del jugador '${row.Player}' para el equipo ${estadistica.idEquipo}. Saltando estadística.`);
                        continue; // Saltar esta estadística si no se encuentra el jugador
                    }

                    // Asegurar que idJornada está actualizado para la estadística
                    const [jornadaDBRows]: any = await db.query('SELECT idJornada FROM Jornada WHERE Nombre = ?', [estadistica.idJornada]);
                    if (jornadaDBRows && jornadaDBRows.length > 0) {
                        estadistica.idJornada = jornadaDBRows[0].idJornada;
                    } else {
                        console.error(`No se encontró el ID de la jornada '${row.Matchweek}'. Saltando estadística.`);
                        continue;
                    }
                    
                    // Asegurar que idPartido está actualizado para la estadística
                    ...
                    if (partidoDBRows && partidoDBRows.length > 0) {
                        estadistica.idPartido = partidoDBRows[0].idPartido;
                         ...
                         console.error(`No se encontró el ID del partido para Jornada=${estadistica.idJornada}, Local=${idEquipoLocal}, Visitante=${idEquipoVisitante}. Saltando estadística.`);
                         continue;
                    }


                    // Verificar si la estadística ya existe para evitar duplicados
                    ...
                    if (!existingEstadisticaRows || existingEstadisticaRows.length === 0) { // Si no existen estadísticas repetidas
                        if (estadistica.idPartido && estadistica.idJugador && estadistica.idJornada && estadistica.idEquipo) {
                           ...
                           // console.log(`Nueva estadística insertada para idJugador=${estadistica.idJugador}, idPartido=${estadistica.idPartido}`);
                            ...
                            console.error('IDs necesarios para insertar estadísticas no encontrados. Saltando.');
                            console.log(`Debug IDs: Partido=${estadistica.idPartido}, Jugador=${estadistica.idJugador}, Jornada=${estadistica.idJornada}, Equipo=${estadistica.idEquipo}`);
                        }
                    } else {
                        //console.log(`Estadística ya existe para idJugador=${estadistica.idJugador}, idPartido=${estadistica.idPartido}. Saltando inserción.`);
                    }
                } catch (error) {
